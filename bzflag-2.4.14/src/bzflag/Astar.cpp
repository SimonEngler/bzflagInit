...
{
	indexShift = (int)(floor(BZDBCache::worldSize / (2 * SCALE) + 1.5f));
	int arraySize = 2 * indexShift + 1; // getNeighbors can return AStarNodes 1 unit outside of world
	closedArray.resize(arraySize);
	for (int i = 0; i < arraySize; ++i)
		closedArray[i].resize(arraySize);
...
void		AStarGraph::startAStar(vector< AStarNode > & path)
{
	...
	while (!openQueue.empty()) {
		...
		for (auto &neighborNode : getSuccessors(currentNode)) {
			...
			if (!neighborNode->isAccessible()) continue; // skip if not accessible
			...
			else if (neighborNode->getStatus() == open1) {
				if (neighborNode->getCostSoFar() <= (currentNode->getCostSoFar() + incrementalCost)) continue;
